...
export const getOptionSetByCatID = async (categoryID: string) => {
  if (!categoryID || categoryID === "") return { error: " Проверка неверных данных еще раз!" };
...
export const addOptionSet = async (data: TOptionSet) => {
  if (!AddOptionSet.safeParse(data).success) return { error: " Проверка неверных данных еще раз" };
...
export const deleteOptionSet = async (id: string) => {
  if (!id || id === "") return { error: " Проверка неверных данных еще раз" };
...
export const addSingleOption = async (data: TSingleOption) => {
  if (!SingleOption.safeParse(data).success) return { error: " Проверка неверных данных еще раз!" };
...
export const addSpecGroup = async (data: TSpecGroup) => {
  if (!AddSpecGroup.safeParse(data).success) return { error: " Проверка неверных данных еще раз" };
...
export const addSingleSpec = async (data: TSingleSpec) => {
  if (!SingleSpec.safeParse(data).success) return { error: " Проверка неверных данных еще раз!" };
  try {
...
export const deleteSingleSpec = async (data: TSingleSpec) => {
  ...
  try {
    ...
    const filteredList = (specsList.specs as string[]).filter((spec) => spec !== data.value);
