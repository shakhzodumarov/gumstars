...
export const deleteCategory = async (id: string) => {
  if (!id) return { error: "Не удается удалить, проверьте еще раз внимательно!" };
  ...
  try {
    ...
    return { error: "Внутри есть данные, поэтому обязательно удалите данные внутри и повторите попытку!" };
  } catch (error) {
    return { error: "Не удается удалить, проверьте еще раз внимательно!" };
  }
};
