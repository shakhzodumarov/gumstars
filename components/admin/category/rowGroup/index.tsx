...
const RowCatGroup = ({ data, categories, onReset }: IProps) => {
  ...
  return (
    <div className={styles.catGroupRow}>
      ...
      <div>
        <Button
          text="Опции / Технические характеристики"
          onClick={() => setShowOptions(true)}
        />
        <Button text="Добавить категорию" onClick={() => setShowAddCategory(true)} />
      </div>
      <div>
        <Button text="Редактировать" onClick={() => setShowEdit(true)} />
      ...
      {showEdit && (
        <Popup
          ...
          title={`Обновление группыp: ${data.name}`}
        />
      )}
      {showDelete && (
        <Popup
